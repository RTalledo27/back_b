...
use App\Modules\Commerce\Application\Actions\RejectWinnerPayoutApprovalAction;
...
final class RejectWinnerPayoutController
{
    public function __invoke(WinnerPayoutReasonRequest $request, WinnerPayout $payout, RejectWinnerPayoutApprovalAction $action, GetWinnerPayoutQuery $query, IdempotentCommandExecutor $executor): AdminWinnerPayoutResource
    {
        Gate::authorize('reject', $payout);
        $actor = (int) $request->user()?->getKey();
        $key = (string) $request->header('Idempotency-Key');
        $reason = $request->reasonCode();
        $context = IdempotencyContext::make($actor, $request->method(), $request->path(), $key, ['operation' => 'winner_payout.reject', 'payout_id' => $payout->id, 'actor_user_id' => $actor, 'reason_code' => $reason]);
