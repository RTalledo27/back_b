...
use App\Modules\Commerce\Application\Actions\StartWinnerPayoutExecutionAction;
...
final class StartWinnerPayoutExecutionController
{
    public function __invoke(Request $request, WinnerPayout $payout, StartWinnerPayoutExecutionAction $action, GetWinnerPayoutQuery $query, IdempotentCommandExecutor $executor): AdminWinnerPayoutResource
    {
        Gate::authorize('execute', $payout);
        $actor = (int) $request->user()?->getKey();
        $key = (string) $request->header('Idempotency-Key');
        $context = IdempotencyContext::make($actor, $request->method(), $request->path(), $key, ['operation' => 'winner_payout.processing', 'payout_id' => $payout->id, 'actor_user_id' => $actor]);
