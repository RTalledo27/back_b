...
use App\Modules\Commerce\Application\Actions\MarkWinnerPayoutFailedAction;
...
final class MarkWinnerPayoutFailedController
{
    public function __invoke(WinnerPayoutReasonRequest $request, WinnerPayout $payout, MarkWinnerPayoutFailedAction $action, GetWinnerPayoutQuery $query, IdempotentCommandExecutor $executor): AdminWinnerPayoutResource
    {
        Gate::authorize('execute', $payout);
        $actor = (int) $request->user()?->getKey();
        $key = (string) $request->header('Idempotency-Key');
        $reason = $request->reasonCode();
        $context = IdempotencyContext::make($actor, $request->method(), $request->path(), $key, ['operation' => 'winner_payout.failed', 'payout_id' => $payout->id, 'actor_user_id' => $actor, 'reason_code' => $reason]);
