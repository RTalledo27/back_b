...
final class PaymentRejectedNotificationHandlerTest extends TestCase
{
    use LazilyRefreshDatabase;

    private function makeHandler(): PaymentRejectedNotificationHandler
    {
        return $this->app->make(PaymentRejectedNotificationHandler::class);
    }
    ...
    private function setupScenario(): array
    {
        ...
        $order = Order::create([
            'user_id' => $buyer->id, 'game_id' => $game->id, 'status' => 'rejected',
            'subtotal_cents' => 100, 'total_cents' => 100, 'currency' => 'PEN',
        ]);
        $payment = Payment::create([
            'order_id' => $order->id, 'amount_cents' => 100, 'currency' => 'PEN',
            'method' => PaymentMethod::Manual, 'status' => PaymentStatus::Rejected,
        ]);
    ...
    public function test_sends_notification_and_creates_delivery(): void
    {
        ...
        Notification::assertSentTo($buyer, PaymentRejectedNotification::class);
    ...
    public function test_does_not_send_if_already_queued(): void
    {
        ...
        Notification::assertSentToTimes($buyer, PaymentRejectedNotification::class, 1);
    }
    ...
    public function test_marks_failed_if_payment_not_rejected(): void
    {
        Notification::fake();
